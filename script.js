...
// const rolJugadorDiv = document.getElementById('rolJugador'); // Removed/Unused
...
// Guion Noche
const abrirGuionBtn = document.getElementById('abrirGuionBtn');
const modalGuion = document.getElementById('modalGuion');
const cerrarGuionBtn = document.getElementById('cerrarGuionBtn');
const guionLista = document.getElementById('guionLista');
...
let jugadoresProtegidos = []; // Array para guardar estado de protección
...
const rolesIconos = {
    "Lobo": "fab fa-wolf-pack-battalion",
    "Lobo Infectador": "fa-biohazard",
    "El Lobo feroz": "fa-paw",
    "El hombre Lobo Albino": "fa-crow", // O algo blanco/distinto
    "Vidente": "fa-eye",
    "Protector": "fa-shield-halved",
    "Bruja": "fa-flask",
    "Cazador": "fa-crosshairs",
    "Aldeano": "fa-user",
    "La niña": "fa-child",
    "Cúpido": "fa-heart",
    "El tonto del pueblo": "fa-masks-theater",
    "Anciano": "fa-person-cane",
    "Cabeza de turco": "fa-user-injured",
    "Niño salvaje": "fa-paw",
    "Perro Lobo": "fa-dog",
    "La Sirvienta": "fa-broom",
    "El Caballero de la espada oxidada": "fa-khanda",
    "Juez Tartamudo": "fa-gavel",
    "El Zorro": "fa-mask",
    "El Domador de Osos": "fa-paw", // Repetido pero válido
    "Ángel": "fa-dove"
};

// Orden sugerido de despertar (Guion de la Noche)
const nightScriptOrder = [
    "Cúpido",
    "Protector",
    "Vidente",
    "Lobo", // Lobos en general
    "La niña",
    "Bruja",
    "Cazador",
    "Anciano",
...
function cargarPartida() {
    ...
    jugadoresProtegidos = p.jugadoresProtegidos || []; // Cargar protegidos
...
function mostrarTurno() {
    ...
    // rolJugadorDiv.style.display = 'none'; // Removed
    cardContainer.style.display = 'none';
    roleCard.classList.remove('flipped');
    verRolBtn.style.display = 'block'; // Show button again
...
function actualizarListaNarrador() {
    ...
    nombresJugadores.forEach((nombre, i) => {
        ...
        // Botón Escudo (Protección)
        const shieldBtn = document.createElement('button');
        shieldBtn.className = 'btn-shield';
        shieldBtn.innerHTML = '<i class="fa-solid fa-shield-halved"></i>';

        // Verificar si está protegido
        if (jugadoresProtegidos[i]) {
            shieldBtn.classList.add('protected');
        }

        shieldBtn.addEventListener('click', (e) => {
            e.stopPropagation();
            jugadoresProtegidos[i] = !jugadoresProtegidos[i]; // Toggle
            actualizarListaNarrador();
            guardarPartida();
        });
...
verRolBtn.addEventListener('click', () => {
    ...
    // Actualizar icono de la carta (parte trasera)
    const iconClass = rolesIconos[rol] || "fa-user-secret";
    // Buscamos si ya existe un icono en la parte trasera, si no lo creamos
    let iconContainer = document.getElementById('rolIconoBack');
    if (!iconContainer) {
        iconContainer = document.createElement('div');
        iconContainer.id = 'rolIconoBack';
        ...
        nombreRolH2.parentNode.insertBefore(iconContainer, nombreRolH2);
    }
    iconContainer.innerHTML = `<i class="fa-solid ${iconClass}"></i>`;

    verRolBtn.style.display = 'none';
...
roleCard.addEventListener('click', () => {
    if (!roleCard.classList.contains('flipped')) {
        ...
        }, 600); // Show button after flip starts
    }
});
...
// Guion Noche
abrirGuionBtn.addEventListener('click', () => {
    generateNightScript();
    modalGuion.style.display = 'flex';
});

cerrarGuionBtn.addEventListener('click', () => {
    modalGuion.style.display = 'none';
});

window.addEventListener('click', (e) => {
    if (e.target == modalGuion) modalGuion.style.display = 'none';
});

function generateNightScript() {
    guionLista.innerHTML = '';

    // Filtrar roles presentes en la partida
    const rolesPresentes = new Set();
    turnos.forEach((rol, index) => {
        if (jugadoresEstado[index] === "alive") {
            rolesPresentes.add(rol);
        }
    });

    // Casos especiales: "Lobo" agrupa a todos los lobos para su turno de caza
    let hayLobos = false;
    turnos.forEach((rol, index) => {
        if (rol.includes("Lobo") && jugadoresEstado[index] === "alive") hayLobos = true;
    });
    if (hayLobos) rolesPresentes.add("Lobo");

    // Roles que solo actúan la primera noche
    const rolesPrimeraNoche = ["Cúpido", "Perro Lobo", "Niño salvaje", "Juez Tartamudo", "Ángel", "La niña", "Cazador", "Anciano", "El tonto del pueblo", "Cabeza de turco", "La Sirvienta", "El Caballero de la espada oxidada", "El Lobo feroz", "El Zorro", "El Domador de Osos"];

    let stepCount = 1;
    nightScriptOrder.forEach(rol => {
        if (rolesPresentes.has(rol)) {
            ...
            // Icono
            const iconClass = rolesIconos[rol] || "fa-circle";

            // Comentario primera noche
            let extraInfo = "";
            if (rolesPrimeraNoche.includes(rol)) {
                extraInfo = ` <span style="color: var(--accent); font-size: 0.8rem; margin-left: 5px;">(Solo 1ª Noche)</span>`;
            }

            p.innerHTML = `<span style="color: var(--primary); font-weight: bold;">${stepCount}.</span> <i class="fa-solid ${iconClass}"></i> ${rol}${extraInfo}`;

            guionLista.appendChild(p);
            stepCount++;
        }
    });

    if (stepCount === 1) {
        guionLista.innerHTML = '<p style="text-align:center; color:#666;">No hay roles especiales activos para despertar.</p>';
    }
}
